<h1 align="center">Hola 👋, soy MarlonX-a</h1>

<h3 align="center">Fullstack Developer | Backend Focused | Software Engineering</h3>

---

## 🧠 Sobre mí

Soy desarrollador enfocado en construir aplicaciones escalables y APIs robustas.
Me interesa especialmente el backend, arquitectura limpia y bases de datos modernas.

- 🚀 Desarrollo Fullstack
- 🔥 Enfoque fuerte en Backend
- 🧩 Buenas prácticas y arquitectura limpia
- 📚 Aprendizaje continuo

---

## 🛠️ Stack Tecnológico

### Backend
<p>
  <img src="https://skillicons.dev/icons?i=nestjs,django,ts,python" />
</p>

### Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,js" />
</p>

### Base de Datos & Backend as a Service
<p>
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white" />
</p>

### Lenguajes
<p>
  <img src="https://skillicons.dev/icons?i=python,js,ts,cpp" />
</p>

---

## 📊 Estadísticas GitHub

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=MarlonX-a&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=MarlonX-a&theme=tokyonight" />
</p>

---

## 🎯 Actualmente

- 🔐 Construyendo APIs con NestJS
- ⚡ Desarrollando apps modernas con React + Supabase
- 🧠 Mejorando patrones de arquitectura backend

---

## 🎮 Extra

Fuera del código, disfruto jugar Valorant 🎯  
<img src="https://upload.wikimedia.org/wikipedia/commons/f/fc/Valorant_logo_-_pink_color_version.svg" width="90"/>

---

<p align="center">
  <a href="https://github.com/MarlonX-a">
    <img src="https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github" />
  </a>
</p>
